...
ACT2FN = {"gelu": gelu, "relu": torch.nn.functional.relu, "swish": swish, 'inplace_gelu': InplaceGelu()}
...
class BertSelfOutput(nn.Module):
    def __init__(self, config):
        ...
        self.LayerNorm = InplaceLayerNorm(config.hidden_size, eps=1e-12)
        # Todo
        # self.dropout = Combined(config.hidden_dropout_prob)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
...
class BertForQuestionAnswering(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.bert = BertModel(config)
        # check with Google if it's normal there is no dropout on the token classifier of SQuAD in the TF version
        # self.dropout = nn.Dropout(config.hidden_dropout_prob)
