...
#         if isinstance(child_module, torch.nn.Dropout):
#             print("有！！！")
#             setattr(module, name, Combined)
#         else:
...
class Model(nn.Module):
    ...
    def __init__(self, config):
        ...
        path = "bert_structure.txt"
        with open (path, 'wb') as file:
            file.write(f"{self.bert}".encode("utf-8"))
        print(f"模型结构文件保存在{path}")
